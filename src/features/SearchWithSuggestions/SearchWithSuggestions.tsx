...
const SearchWithSuggestions = (props: SearchWithSuggestionsProps) => {
  ...
  return (
    <div className={cl.searchContent} ref={outsideRef}>
      <SearchInput
        inputRef={inputRef}
        value={value}
      ...
      {isLoading ? (
        ...
        <div className={cl.movieListContainer} ref={containerRef}>
          <ul className={cl.movieList}>
            {(value.trim().length > 0 ? searchedMovies : allMovies).map(
              (movie) => {
                return (
                  <li key={movie.id} className={cl.movieListItem}>
                    <Link to={`/movie/${movie.id}`} className={cl.movieLink}>
                      <div className={cl.posterContainer}>
                        <img
                          src={
                            movie.poster?.previewUrl ||
                            '../public/placehold.png'
                          }
                      ...
                      <div>
                        ...
                        <div className={cl.movieDescriptionRating}>
