...
const MovieCard = (props: MovieCardProps) => {
  const { movieData, seasons, reviews, onPlayClick } = props
  ...
  return (
    <section className={cl.movieContent}>
      <div className={cl.movieInfo}>
        ...
        <Description className={cl.description} movieData={movieData} />
        <div className={cl.castMovieCard}>
          ...
          <SliderCast onSwiper={setSliderCast}>
            {movieData &&
              movieData?.persons?.map((person) => (
                <SwiperSlide className={cl.cardPhotoCast}>
                  <img
                    src={person.photo}
                    alt={person.name}
        ...
        <div className={cl.review}>
          <div className={cl.headerMovieCard}>
            <h6>Reviews</h6>
            <div className={cl.headerMovieCard} ref={outsideRef}>
              {openCreateReview ? (
      ...
      >
          ...
          <div className={cl.sectionList}>
            {showAll
              ? movieData?.names
                  ...
                  .map((name) => <p className={cl.infoTag}>{name.language}</p>)
              : movieData?.names
                  ?.filter((name) => name?.language)
                  .slice(0, defaultShowLanguage)
                  .map((name) => <p className={cl.infoTag}>{name.language}</p>)}
            {movieData?.names?.filter((name) => name?.language).length >
              defaultShowLanguage &&
              !showAll && (
                <ButtonIcon
                  label={<More />}
        ...
        <div className={cl.infoSection}>
          ...
          <div className={cl.sectionList}>
            {movieData?.rating &&
              Object.entries(movieData.rating)
                .filter(([key]) => key === 'imdb' || key === 'kp')
                .map(([key, value]: [keyof Rating, number]) => (
                  <div className={cl.raitingTag}>
                    <h6 key={key}>{key}</h6>
                    <div className={cl.raitingTagBody}>
                      <Ratings totalStars={10} ratingValue={value} />
                ...
                return (
                  <div className={cl.directorSection}>
                    <img
                      ...
                      alt={person.name}
                    />
                    <p key={person.id}>{person.name ? person.name : null}</p>
                  </div>
                )
              })}
        </div>
