...
const ComparePlan = () => {
  ...
  const [billingPlant, setBillingPlant] = useState<
        ...
        {tablet ? (
            ...
            <thead className={cl.planTableHead}>
              <tr className={cl.planTableHeaderRow}>
                ...
                {dataComparePlan.map((comparePlan) => (
                  <th className={cl.planTableHeader} key={comparePlan.id}>
              ...
              {Object.keys(dataComparePlan[0].features).map((feature) => (
                <tr>
                  ...
                  {dataComparePlan.map((plan) => (
                    <td className={cl.planTableBody} key={plan.id}>
          ...
          <div>
            <Tab
              ...
              onChange={(id) => {
                setBillingPlant(
                  id === 1 ? 'Basic' : id === 2 ? 'Standard' : 'Premium',
                )
              }}
            />
            <ComparePlanCard
              plan={billingPlant}
              dataComparePlan={dataComparePlan}
            />
          </div>
        )}
      </div>
