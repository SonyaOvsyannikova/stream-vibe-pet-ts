...
export const kinopoiskAPI = {
  ...
  getSortedMoviesWithParametersSlug: async (
    slug: string,
    options: listOptions = {},
  ) => {
    try {
  ...
  getMoviesSortedByBackdropAndTopMovies: async (
    ...
    try {
      const params = {
        ...
        notNullFields: ['backdrop.url', 'poster.url', 'rating.kp'],
        year: '2024-2025',
      }
  ...
  getReviewedMovies: async (
    id: string | number,
    options: { signal?: AbortSignal } = {},
  ) => {
    try {
      const response = await axiosInstance.get(`/review?movieId=${id}`, {
        signal: options.signal,
      })
      return response.data
    } catch (error) {
  ...
  getSeasons: async (
    ...
    try {
      const response = await axiosInstance.get(`/season?movieId=${id}`, {
        signal: options.signal,
      })
      return response.data
    } catch (error) {
