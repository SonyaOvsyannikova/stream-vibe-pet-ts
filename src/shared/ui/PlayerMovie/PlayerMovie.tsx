...
const PlayerMovie = (props: PlayerMovieProps) => {
  const { selectedEpisodes, movieData } = props
    ...
    <div className="container">
      {selectedEpisodes && (
        ...
        >
          ...
          <p>
            Эпизод {selectedEpisodes.episode?.number}:{' '}
            {selectedEpisodes.episode?.name}
          </p>
        </div>
      )}
      <video
        ...
        style={{ width: '100%' }}
        src={
          movieData?.type === 'tv-series' ? '/planet.mp4' : '/planet_one.mp4'
        }
      ></video>
    </div>
  )
