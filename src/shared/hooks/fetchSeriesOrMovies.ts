...
export const fetchSeriesOrMovies = async (
  slug: string,
  categorySlug: string,
  categoryName: string,
) => {
  ...
  const arrId: number[] = response.docs.map(
    (item: MovieWithCategory): number => item.movie.id,
  )
  ...
  const combinedData: MovieWithCategory[] = response.docs.map(
